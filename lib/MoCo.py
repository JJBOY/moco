...
class MoCo(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, K=65536, m=0.999, T=0.07, wrap=False, symmetric=False,
                 projector=False, predictor=False):
        """
        ...
        if predictor:
            self.predictor = PredictionMLP(dim, 512, dim)
        else:
    ...
    def forward(self, im_q, im_k):
        ...
        if self.symmetric:  # asymmetric loss
            loss_12, k2 = self._contrastive_loss(im_q, im_k)
            loss_21, k1 = self._contrastive_loss(im_k, im_q)
            loss = loss_12 + loss_21
            k = torch.cat([k1, k2], dim=0)
        else:  # asymmetric loss
            loss, k = self._contrastive_loss(im_q, im_k)
